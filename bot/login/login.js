...
// ——— SESSION GUARD ——————————————————————————————————————————————————————————
// Saves a timestamped backup of appState to ypb_fca_database/session_backup.json
// so that on restart the freshest cookies are always used, not the stale APPSTATE env var.
const SESSION_BACKUP_PATH = require("path").join(process.cwd(), "ypb_fca_database", "session_backup.json");

function saveSessionBackup(appStateData) {
        try {
                require("fs-extra").ensureDirSync(require("path").dirname(SESSION_BACKUP_PATH));
                writeFileSync(SESSION_BACKUP_PATH, JSON.stringify({
                        savedAt: Date.now(),
...
function loadSessionBackup() {
        try {
                if (!existsSync(SESSION_BACKUP_PATH)) return null;
                const data = JSON.parse(readFileSync(SESSION_BACKUP_PATH, "utf8"));
                if (!data.appState || !Array.isArray(data.appState) || data.appState.length === 0) return null;
                return data;
        } catch {
                return null;
        }
...
async function getAppStateToLogin(loginWithEmail) {
        ...
        // ——— SESSION GUARD: prefer session_backup.json if it is newer than account.txt ———
        try {
                const backup = loadSessionBackup();
                if (backup && backup.savedAt) {
                        const accountMtime = existsSync(dirAccount) ? require("fs").statSync(dirAccount).mtimeMs : 0;
                        if (backup.savedAt > accountMtime + 5000) {
                                // backup is genuinely newer → restore it to account.txt first
                                writeFileSync(dirAccount, JSON.stringify(backup.appState, null, 2));
                                log.info("SESSION GUARD", `Restored fresher session backup (saved ${new Date(backup.savedAt).toISOString()})`);
                        }
                }
        } catch { /* non-fatal */ }
        // ———————————————————————————————————————————————————————————————————
                                ...
                                if (facebookAccount.email && facebookAccount.password) {
                                        ...
                                        setTimeout(async function refreshCookie() {
                                                try {
                                                        ...
                                                        saveSessionBackup(filterKeysAppState(appState));
                                                        setTimeout(() => changeFbStateByCode = false, 1000);
