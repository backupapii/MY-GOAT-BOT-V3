...
const nodeBuiltins = new Set([
        ...
        "crypto","dgram","diagnostics_channel","dns","domain","events","fs","fs-extra",
        ...
        for (const folderModules of folders) {
                ...
                for (const file of Files) {
                        ...
                        try {
                                ...
                                if (allPackage) {
                                        ...
                                        for (let packageName of allPackage) {
                                                ...
                                                if (!packageAlready.includes(packageName)) {
                                                        ...
                                                        if (!nodeBuiltins.has(packageName) && !existsSync(`${process.cwd()}/node_modules/${packageName}`)) {
                                                                ...
                                                                try {
                                                                        ...
                                                                        clearInterval(wating);
                                                                        process.stderr.write('\r\x1b[K');
                                                                        console.log(`${colors.red('✖')} installed package ${packageName} failed — skipping (require() will catch it)`);
                                                                        // Non-fatal: don't throw, let require() below fail instead and skip just this command
                                                                }
                                                        }
                                                }
                                        }
                                ...
                                // ——————————————— CHECK SYNTAXERROR ——————————————— //
                                // MUST check configCommand exists before accessing any property
                                if (!configCommand || typeof configCommand !== "object" || Array.isArray(configCommand))
                                        throw new Error(`config of ${text} must be a plain object (got ${configCommand === null ? "null" : typeof configCommand})`);
                                const commandName = configCommand.name;
                                if (!commandName || typeof commandName !== "string")
                                        throw new Error(`name of ${text} must be a non-empty string`);
                                // category is optional — auto-assign "misc" instead of crashing
                                if (!configCommand.category)
                                        configCommand.category = "misc";
                                if (!command.onStart)
                                        throw new Error(`onStart of ${text} undefined`);
                                if (typeof command.onStart !== "function")
                                        throw new Error(`onStart of ${text} must be a function`);
                                // Duplicate: log warning + override (latest wins) instead of throwing
                                if (GoatBot[setMap].has(commandName)) {
                                        log.warn("LOAD", `Duplicate ${text} "${commandName}" found in "${file}" — overriding previous version`);
                                        const oldCmd = GoatBot[setMap].get(commandName);
                                        if (Array.isArray(oldCmd?.config?.aliases))
                                                for (const alias of oldCmd.config.aliases)
                                                        if (GoatBot.aliases.get(alias) === commandName.toLowerCase())
