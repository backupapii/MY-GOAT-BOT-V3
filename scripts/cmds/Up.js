...
ensureFont("CourierPrime-Regular.ttf", "https://github.com/google/fonts/raw/main/ofl/courierprime/CourierPrime-Regular.ttf");
ensureFont("CourierPrime-Bold.ttf", "https://github.com/google/fonts/raw/main/ofl/courierprime/CourierPrime-Bold.ttf");
ensureFont("CourierPrime-Italic.ttf", "https://github.com/google/fonts/raw/main/ofl/courierprime/CourierPrime-Italic.ttf");
ensureFont("CourierPrime-BoldItalic.ttf", "https://github.com/google/fonts/raw/main/ofl/courierprime/CourierPrime-BoldItalic.ttf");
...
module.exports = {
  config: {
    ...
    countDown: 10,
    role: 0,
    shortDescription: { en: "Bot uptime & system monitor UI" },
    longDescription:  { en: "Displays bot uptime, CPU, RAM, disk in a Windows 11-style card." },
    category: "system",
    guide: { en: "{pn} [dark|light]" }
  },
  ...
  onStart: async function ({ message, event, args }) {
    try {
      ...
      const themeName = (args[0] || "dark").toLowerCase() === "light" ? "light" : "dark";
      const th = THEMES[themeName] || THEMES.dark;

      const cpuPct  = getCPU();
      await new Promise(r => setTimeout(r, 300));
      const cpuFinal = Math.max(getCPU(), cpuPct);

      const totMem  = os.totalmem();
      const freeMem = os.freemem();
      const usedMem = totMem - freeMem;
      const ramPct  = Math.round((usedMem / totMem) * 100);
      const ramGB   = (totMem  / 1024 ** 3).toFixed(1);
      const usedGB  = (usedMem / 1024 ** 3).toFixed(1);

      const diskPct   = getDisk();
      ...
      const cpuModel = (cpus[0]?.model || "Unknown CPU").replace(/\s+/g, " ").substring(0, 30);
      const loadArr  = os.loadavg();
      const load     = loadArr.map(l => l.toFixed(2)).join(" ");

      const uptimeSec = process.uptime();
      const uh = Math.floor(uptimeSec / 3600);
      const um = Math.floor((uptimeSec % 3600) / 60);
      const us = Math.floor(uptimeSec % 60);
      const uptime = `${String(uh).padStart(2,"0")}h ${String(um).padStart(2,"0")}m ${String(us).padStart(2,"0")}s`;

      let sysStatus = "Online", sysStatusColor = "#34d399";
      if (ramPct > 85 || cpuFinal > 85) { sysStatus = "High Load"; sysStatusColor = "#f59e0b"; }
      if (ramPct > 95 || cpuFinal > 95) { sysStatus = "Critical";  sysStatusColor = "#ef4444"; }

      const sysData = {
        cpu: cpuFinal, ram: ramPct, disk: diskPct,
        cpuModel, threads, ramGB, usedGB,
        platform: os.platform(), arch: os.arch(),
        hostname: os.hostname().substring(0, 18),
        uptime, load, temp, network,
        netIPs, diskTotal, diskUsed,
        ping: 0, pingLabel: "N/A", pingAccent: "#34d399",
        sysStatus, sysStatusColor, themeName
      };

      const imgBuf  = await renderPage("overview", sysData, th);
      const tmpDir  = path.join(__dirname, "cache");
      require("fs-extra").mkdirpSync(tmpDir);
      const tmpPath = path.join(tmpDir, `up_${Date.now()}.png`);
      fs.writeFileSync(tmpPath, imgBuf);

      await message.reaction("✅", event.messageID);
      await message.reply({ body: "", attachment: fs.createReadStream(tmpPath) });
      setTimeout(() => { try { fs.unlinkSync(tmpPath); } catch (_) {} }, 12000);
