...
const API_ENDPOINT = "https://betadash-api-swordslush-production.up.railway.app/upscale";
...
module.exports = {
  ...
  onStart: async function ({ args, message, event }) {
    ...
    try {
      ...
      const fullApiUrl = `${API_ENDPOINT}?imageUrl=${encodeURIComponent(imageUrl)}`;

      // 2. Call the API to get the upscaled image URL
      const apiResponse = await axios.get(fullApiUrl, { timeout: 60000 });
      const data = apiResponse.data;

      const upscaledImageUrl = data.imageUrl || data.image || data.url || data.result || (data.ok && data.data?.imageUrl);
      if (!upscaledImageUrl) {
