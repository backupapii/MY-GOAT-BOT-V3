...
// ── Translation fallback chain (3 sources) ───────────────────────────────────
async function translate(text, langCode) {
  // Source 1: Google Translate unofficial endpoint (fast, no key)
  try {
  ...
  // Source 2: MyMemory API (free, 1000 words/day per IP)
  try {
    const res = await axios.get(
      `https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=auto|${langCode}`,
      { timeout: 10000 }
    );
  ...
  try {
    ...
    if (translated) return { text: translated, lang: "auto" };
    throw new Error("empty");
  } catch (_) {}

  // Source 4: Lingva Translate (open source Google Translate frontend)
  try {
    const res = await axios.get(
      `https://lingva.ml/api/v1/auto/${langCode}/${encodeURIComponent(text)}`,
      { timeout: 10000 }
    );
  ...
  throw new Error("All 4 translation APIs failed. Try again later.");
}
...
async function translateAndSendMessage(content, langCodeTrans, message, getLang) {
  try {
    ...
    return message.reply(`❌ Translation failed: ${err.message}`);
  }
}
...
module.exports = {
        config: {
                ...
                description: {
                        ...
                        en: "Translate text to the desired language (4 fallback APIs)"
                },
                category: "utility",
                guide: {
