				...
				try {
					...
					try {
						...
						try {
							...
							const thumbSrc = userInfo[leftParticipantFbId]?.thumbSrc
								|| `https://graph.facebook.com/${leftParticipantFbId}/picture?width=512&height=512`;
							const r = await axios.get(thumbSrc, { responseType: 'arraybuffer', timeout: 7000 });
							avatarBuffer = Buffer.from(r.data);
						} catch (_) {
							const r = await axios.get(`https://graph.facebook.com/${leftParticipantFbId}/picture?width=512&height=512`, { responseType: 'arraybuffer', timeout: 7000 });
							avatarBuffer = Buffer.from(r.data);
						}
