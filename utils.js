...
const _hasGoogleCreds = !!(clientId && clientSecret && refreshToken);
if (!_hasGoogleCreds) {
        log.warn("CREDENTIALS", "Google credentials not configured in config.json. Google Drive and Gmail features will be disabled.");
}
        ...
        return {
                send: async (form, callback) => {
                        try {
                                ...
                                return await api.sendMessage(form, event.threadID, callback, null, event.isGroup);
                        }
                        catch (err) {
                ...
                reply: async (form, callback) => {
                        try {
                                global.statusAccountBot = 'good';
                                return await api.sendMessage(form, event.threadID, callback, event.messageID, event.isGroup);
                        }
                        catch (err) {
                                if (JSON.stringify(err).includes('spam')) {
                                        setErrorUptime();
